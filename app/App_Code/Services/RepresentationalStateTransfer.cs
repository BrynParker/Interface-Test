...
using RiverlandIndustries.Security;
        ...
        protected virtual void CombineScripts(HttpContext context, bool caching, bool isSaaS, string culture)
        {
            ...
            if (isSaaS)
            {
                if (ApplicationServices.IsTouchClient)
                    ...
                    sb.AppendFormat(string.Format("$('<link></link>').appendTo($('head')).attr({{ href: '{0}/App_Themes/RiverlandIndustries/_Theme_Cay.css?{0}', type: 'text/css', rel: 'stylesheet' }});", ApplicationServices.Version), baseUrl);
                try
                {
                ...
                sb.AppendFormat("var __targetFramework='4.8';__tf=4.0;__cothost='appfactory';__appInfo='RL_Interface_TKS|{0}';", BusinessRules.JavaScriptString(context.User.Identity.Name));
        ...
        protected virtual void BeginResponsePadding(HttpRequest request, StreamWriter sw)
        {
            ...
            else
            {
                if ((request.HttpMethod == "GET") && UriRestConfig.IsJSONPRequest(request))
                {
                    ...
                    sw.Write("RiverlandIndustries=typeof RiverlandIndustries=='undefined'?{{}}:RiverlandIndustries;RiverlandIndustries.{0}=", instance);
                }
            }
        }
