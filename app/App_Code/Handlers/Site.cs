...
namespace RiverlandIndustries.Handlers
{
    public partial class Site : SiteBase
    {
    }

    public class SiteBase : RiverlandIndustries.Web.PageBase
    {
        ...
        public static string Copyright
        {
            get
            {
                return "&copy; 2024 RiverlandIndustries. ^Copyright^All rights reserved.^Copyright^";
            }
        }
        ...
        protected virtual void InitializeSiteMaster()
        {
            ...
            var themePath = Server.MapPath("~/App_Themes/RiverlandIndustries");
            if (Directory.Exists(themePath))
                foreach (var stylesheetFileName in Directory.GetFiles(themePath, "*.css"))
                {
                    var fileName = Path.GetFileName(stylesheetFileName);
                    if (!fileName.Equals("_Theme_Cay.css"))
                    {
                        var link = new HtmlLink()
                        {
                            Href = ("~/App_Themes/RiverlandIndustries/" + fileName)
                        };
